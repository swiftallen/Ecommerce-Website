...
if (!isset($_SESSION['name'])) {
    ...
    if (is_numeric($id) == true) {
        ...
        while ($catarray = mysqli_fetch_array($result)) {
            ...
            $catImg = $catarray['catimg'];
            $catName = $catarray['catname'];
            $catProducts = $catarray['catproducts'];
        }
    } else {
    ...
    <div class="container mt-4">
        <form action="" method="post" enctype='multipart/form-data'>
            <div class="mb-3">
                ...
                <input type="text" class="form-control" value="<?php echo $catId; ?>" disabled readonly>
            </div>
            <div class="mb-3">
                <label for="categoryImage" class="form-label">Category Image</label>
                <input type="text" class="form-control" value="<?php echo $catImg; ?>" name="cimg">
                <img src="../<?php echo $catImg; ?>" width="40px" height="40px">
                <label for="formFileMultiple" class="form-label">Upload New Image to Overwrite Previous Image</label>
                <input type="file" class="form-control" name="file">
            </div>
            <div class="mb-3">
                <label for="categoryName" class="form-label">Category Name</label>
                <input type="text" class="form-control" name='cname' value="<?php echo $catName; ?>">
            </div>
            <div class="mb-3">
                <label for="categoryproducts" class="form-label">Category Products</label>
                <input type="text" class="form-control" name='cproducts' value="<?php echo $catProducts; ?>">
            </div>
            <div class="mb-3">
        ...
        if (isset($_POST['submit'])) {
            ...
            $cat_img = $_POST['cimg']; //initialize variable, global local issue
            ...
            if (!empty($_FILES['file']['name'])) {
                ...
                $cat_image = $_POST['cimg']; //fallback to manual path entry
            }
            //image upload code ends

            $catname = $_POST['cname'];
            $catProducts = $_POST['cproducts'];

            $updateQuery = "UPDATE categories SET catimg = '" . $cat_image . "',
        catname='" . $catname . "', catproducts='" . $catProducts . "' WHERE catid=" . $id;
            ...
            window.location.href='manageCat.php'</script>";
                ...
                echo "<script>alert('Error! Unable to Update Category');
            window.location.href='manageCat.php'</script>";
            }
        }
        ?>
