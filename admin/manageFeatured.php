                    ...
                    <div class="modal-body">
                        <form action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="mb-3"><select class="form-select" aria-label="Default select example" name='categoryname' required>
                                    ...
                                    ?>
                                        <option value="<?php echo $count ?>"><?php echo $catArray['catname'] ?></option>
                            ...
                            <div class="mb-3">
                                <label for="imagefile" class="form-label">Upload Featured Product Image</label>
                                <input type="file" class="form-control" name="file" required>
                            </div>
                            <div class="mb-3">
                                <label for="name" class="form-label">Featured Product Name</label>
                                <input type="text" class="form-control" name="name" placeholder="Featured Product Name" required>
                            </div>
                            <div class="mb-3">
                                <label for="newprice" class="form-label">Featured Product New Price</label>
                                <input type="number" class="form-control" name="newprice" placeholder="Product New Price" required>
                            </div>
                            <div class="mb-3">
                                <label for="oldprice" class="form-label">Featured Product Old Price</label>
                                <input type="number" class="form-control" name="oldprice" placeholder="Product Old Price" required>
                            </div>
                            <div class="mb-3">
                            ...
                            if (isset($_POST['insert'])) {
                                ...
                                if (!empty($_FILES['file']['name'])) {
                                    ...
                                    $targetDir = '../img/';
                                    $imgUploadPath = $targetDir . $name_of_image;
                                ...
                                $myQuery = "INSERT INTO featuredproduct (catid,img,name,newprice,oldprice,
                                star_rating1,star_rating2,star_rating3,star_rating4,star_rating5,reviews) 
                                VALUES('" . $categoryname . "', '" . $img . "', '" . $name . "', '" . $newprice . "', '" . $oldprice . "', 
                                '" . $star_rating1 . "', '" . $star_rating2 . "', '" . $star_rating3 . "', '" . $star_rating4 . "'
                                , '" . $star_rating5 . "', '" . $reviews . "')";
                                $result = mysqli_query($conn, $myQuery);

                                if ($result) {
                                    echo "<script>alert('New Featured Product Inserted Successfully');
                                    window.location.href='manageFeatured.php'</script>";
                                    // location.reload()
                                } else {
                                    echo "<script>alert(Error! Unable to upload new featured product.);
                ...
                <tbody class="table-group-divider">
                    ...
                    ?>
                        <tr>
                            ...
                            <td><img src="../<?php echo $prArray['img'] ?>" width='60px' height='60px'></td>
                            <td><?php echo $prArray['name'] ?></td>
                            <td><?php echo $prArray['newprice'] ?></td>
                            <td><?php echo $prArray['oldprice'] ?></td>
                            <td>
                            ...
                            <td>
                                ...
                                <a href="editFeatured.php?id=<?php echo $prArray['id']; ?>" title="Edit/Update"><ion-icon name="create-outline"></ion-icon>
                                <a class="deleteFeatured" title='Delete' id=<?php echo $prArray['id']; ?>><ion-icon name="trash-outline" title='Delete'></ion-icon>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
            ...
            if (confirm('Are you sure you want to delete entry ' + id + '?')) {
                $.ajax({
                    ...
                    success: function(data) {
                        $("#" + id).remove();
                        alert("Featured Product Successfuly Deleted");
                        // alert(data);
                    }
                });
            } else {
